...
def find_lua_type_from_cpp_type(lua_db, cpp_type_name):
    all_base_elements = merge_dicts(lua_db.functions, lua_db.variables, lua_db.classes)
    elements = get_all_lua_elements(all_base_elements)
    for lua_element in elements:
        if lua_element["reference"] in [
            cpp_type_name,
            cpp_type_name.removeprefix("obe::"),
        ]:
            return lua_element
...
LUA_TYPE_MATCH = {"kaguya::LuaTable": "table", "kaguya::LuaFunction": "function"}
SFML_TYPE_MATCH = {
    ...
    for lua_class in all_classes:
        for method in lua_class["methods"].values():
            if method["function_ptr"].endswith("_wrapper()"):
                ...
                splitted_cpp_function_ptr = (
                    method["function_ptr"].removeprefix("&").split("<")[0].split("::")
                )
                ...
                if lua_return_type is None:
                    ...
                    log.warning(
                        f"Unknown Return Type '{lua_return_type}' for {lua_class['name']}::{cpp_method_name}"
                    )
                method["return_type"] = lua_return_type
                method["parameters"] = {}
                for cpp_method_parameter_name, cpp_method_parameter in cpp_method[
                    "parameters"
                ].items():
                    # lua_method_parameter_type =
                    lua_parameter_type = cpp_type_to_lua_type(
                        lua_db, cpp_method_parameter["type"]
                    )
                    if lua_parameter_type is None:
                        lua_parameter_type = cpp_method_parameter["type"]
                        log.warning(
                            f"Unknown Parameter Type '{lua_parameter_type}' for {lua_class['name']}::{cpp_method_name}"
                        )
                    ...
                    if "description" in cpp_method_parameter:
                        method["parameters"][cpp_method_parameter_name][
                            "description"
                        ] = cpp_method_parameter["description"]
    return all_classes
...
def transform_all_cpp_types_to_lua_types(lua_db):
    ...
    for lua_class in all_classes:
        for method in lua_class["methods"].values():
            if "return_type" in method and isinstance(
                method["return_type"], FutureLuaReferenceTag
            ):
                lua_type = cpp_type_to_lua_type(
                    lua_db, method["return_type"].cpp_type, True
                )
                if lua_type:
                    ...
                    log.warning(
                        f"Can't find reference to type '{method['return_type']}' !"
                    )
